...
export function useTweaks(defaults: TweakValues): [TweakValues, (keyOrEdits: string | Partial<TweakValues>, val?: unknown) => void] {
  const [values, setValues] = useState<TweakValues>(defaults)
  const setTweak = useCallback((keyOrEdits: string | Partial<TweakValues>, val?: unknown) => {
...
export function TweakRadio({ label, value, options, onChange }: {
  ...
  const labelLen = (o: typeof options[0]) => String(typeof o === 'object' ? o.label : o).length
  const maxLen = options.reduce((m, o) => Math.max(m, labelLen(o)), 0)
  const fitsAsSegments = maxLen <= (({ 2: 16, 3: 10 } as Record<number, number>)[options.length] ?? 0)

  if (!fitsAsSegments) {
    return (
      <TweakSelect label={label} value={value}
